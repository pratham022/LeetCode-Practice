...
Node* LCA(Node *root, int n1, int n2)
{
    ...
    // the first node which lies in between the two numbers n1 and n2 is the LCA of the nodes, 
    ...
    while(!found) {
        ...
        if(n1 > p->data && n2 > p->data) {
            // both n1 and n2 are greater than the current, so go to right
            p = p->right;
        }
        else if(n1 < p->data && n2 < p->data) {
            // both n1 and n2 are less than the current, so go to left
            p = p->left;
        }
        else {
            // this is the case when root->data lies between n1 and n2
            // so, stop traversing
            found = true;
        }
    }
